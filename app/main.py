...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .toolbar { display: grid; grid-template-columns: minmax(0, 1fr) auto auto; align-items: center; gap: 18px; padding: 86px 0 68px; }
        ...
        button { max-width: 100%; border-radius: 3px; padding: 22px 34px; background: transparent; border: 2px solid var(--accent); color: var(--accent); font-size: clamp(18px, 1.5vw, 32px); font-weight: 700; cursor: pointer; }
        button:disabled { opacity: .55; cursor: wait; }
        .button-primary { background: var(--accent); color: #05040a; }

        .content-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: clamp(28px, 4vw, 72px); max-width: 100%; }
        .panel { min-width: 0; }
...
<body>
    ...
    <script>
        ...
        function emptyMarkup(label) { return '<div class="empty-state">Keine ' + esc(label || 'Nachrichten') + ' gefunden</div>'; }
        ...
        async function loadFolder(folder, targetElementId, label) {
            ...
            try {
                ...
                var data = await response.json();
                listEl.innerHTML = data.items && data.items.length ? data.items.map(function(item, index) { return createMessageItemHtml(item, folder, targetElementId + '-' + index); }).join('') : emptyMarkup(label);
                updatePanelCounts();
            } catch (error) {
                console.error('Error loading folder:', folder, error);
                listEl.innerHTML = targetElementId === 'draftsList' ? emptyMarkup('Entwürfe') : '<div class="error-message">⚠️ Fehler beim Laden der Nachrichten<br>Die Nachrichten konnten nicht geladen werden. Bitte versuchen Sie es erneut.</div>';
                updatePanelCounts();
            }
        }
        ...
        async function loadAllFolders() {
            ...
            try {
                await Promise.all([loadFolder(INBOX_FOLDER, 'inboxList', 'Nachrichten'), loadFolder(DRAFTS_FOLDER, 'draftsList', 'Entwürfe'), loadFolder(SENT_FOLDER, 'sentList', 'Nachrichten')]);
                await loadSummary();
            } catch (error) {
